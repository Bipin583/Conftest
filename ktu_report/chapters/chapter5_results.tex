...
\section{Threat to Validity: Stale Artifacts}
The committed ensemble was trained with LightGBM's \texttt{subsample} parameter set but
\texttt{subsample\_freq} left at its default of $0$, which silently disables row bagging. The five seeds
therefore differed only in column sampling and tie-breaking, making the reported epistemic $\sigma$ an
underestimate of true ensemble diversity. The training code has been corrected, but no artifact in
\texttt{reports/} or \texttt{models/} has been regenerated. Since $\sigma$ is precisely the quantity the
abstention gate thresholds, the operating points in this chapter may shift once the ensemble is
retrained, and every figure here should be read as belonging to the pre-fix ensemble.
